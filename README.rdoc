...
6. Globally update the descriptions of all images in your dabase. This WILL make
   ...
   You can do this by clicking the 'Update all descriptions' button on the options
   page (NOT IMPLEMENTED YET). A means of doing this on a per-gallery basis is
   provided under 8.
...
* For each matching file, the link for the thumbnail when viewing a gallery
  ...
  JW Player [3] and Flowplayer [4], but only using Lightview [5] to view the
  gallery, so this may or may not work for you at all.
...
* Only tested with Lightview. Other lightbox clones almost guaranteed not to work.
...
* After importing, the filename without its image extension (in the example:
  'myvideo.mp4') will be entered as the 'Alt & Title Text'. This is NextGEN
  Gallery's default behaviour. This value should not be changed, because NextGEN
  Enhancer's javascript will use this value to create the link to the video
  file. If you change the Alt & Title Text, the link may become broken.
* The format of the XML file is fixed for now. This should change in the future.
* This feature only works if 'Automatic description management' is active, or you
  had NextGEN Enhancer assemble the description by hand, as described under 8 above.
  The reason is, that the description should start with the word 'Video'. NextGEN
  Enhancer's javascript uses that to match the videos and change the links.
* For Lightview, the description template for videos (see options page) should
  contain the tag {lightviewoptions} at the end. This tag will make sure that
  the description is formatted, so that Lightview knows the dimensions of the video.
